...
def filter_palette_sem(sem, labels=[4,6,7,8,10]):
    resem = np.zeros(sem.shape[:2])
    for i, label in enumerate(labels):
        resem[SEM_COLORS_CONVERT[label]==sem] = i+1
    return resem
...
def rgb2mask(img):
    ...
    for i, c in enumerate(values):
        try:
            mask[img_id==c] = SEM_COLORS_CONVERT_INV[tuple(img[img_id==c][0])] 
        except:
            pass
    return mask
