...
class RAILS:
    ...
    def encoder_state_dict(self):
        if self.multi_gpu:
            return self.autoencoder.backbone_wide.module.state_dict()
        else:
            return self.autoencoder.backbone_wide.state_dict()
    ...
    def train_encoder(self, wide_rgbs, wide_sems, spds):
        ...
        decoder_loss = F.mse_loss(decoded_rgb, wide_rgbs)

        loss = decoder_loss + seg_loss
        ...
        return dict(
            decoder_loss=float(decoder_loss),
            seg_loss=float(seg_loss),
            combined_loss=float(loss),
            decoded_rgb=to_numpy(decoded_rgb[0].permute(1,2,0)),#.byte()),
            gt_seg=to_numpy(wide_sems[0]),
            pred_seg  =to_numpy(wide_seg_outputs[0]).argmax(0),
            wide_rgb=to_numpy(wide_rgbs[0].permute(1,2,0))#.byte()),
        )
    ...
    @torch.no_grad()
    def evaluate_autoencoder(self, wide_rgbs, wide_sems, spds):
        wide_rgbs = wide_rgbs.float().permute(0,3,1,2).to(self.device)
        wide_sems = wide_sems.long().to(self.device)
