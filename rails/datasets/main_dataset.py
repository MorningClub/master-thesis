...
from utils import filter_sem, filter_palette_sem, rgb2mask
...
class RemoteMainDataset(MainDataset):
    def __init__(self, *args, **kwargs):
        MainDataset.__init__(self, *args, **kwargs)
        self.act_vals = dict()
...
class ReformattedMainDataset(Dataset):
    def __init__(self, data_dir, config_path):
        super().__init__()
        ...
        for full_path in tqdm(glob.glob(f'{data_dir}/**')):
            with open(f'{full_path}/data.json') as f:
    ...
    def __getitem__(self, idx):
        ...
        file_path = self.file_map[idx]#[index]

        #with open(f'{file_path}/data.json', "r") as f:
        #    json_file = json.load(f)

        wide_rgb = io.imread(f'{file_path}/rgbs/wide_{cam_index}_{index:05d}.jpg').reshape(240,480,3)
        wide_sem = io.imread(f'{file_path}/rgbs/wide_sem_{cam_index}_{index:05d}.png').reshape(240,480, 4)
        #narr_rgb = io.imread(f'{file_path}/rgbs/narr_{cam_index}_{index:05d}.jpg').reshape(240,384,3)
        #narr_sem = io.imread(f'{file_path}/rgbs/narr_sem_{cam_index}_{index:05d}.png').reshape(240,384,4)

        wide_sem = rgb2mask(wide_sem)
        #narr_sem = rgb2mask(narr_sem)

        #Do i need this?
        wide_sem = filter_sem(wide_sem, self.seg_channels)

        #cmd = json_file[str(index)]['cmd'][0]
        #spd = json_file[str(index)]['spd'][0]

        #act_val = np.array(json_file[str(index)][f'act{cam_index}']).reshape(6,self.num_steers*self.num_throts+1 ,self.num_speeds)
